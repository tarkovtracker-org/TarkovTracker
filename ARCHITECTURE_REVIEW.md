...
7. Acquire a single-writer promotion lock with a unique owner token and lease. Record the pre-promotion values `active-release = A` and `previous-release = P`. Before changing either pointer, revalidate the owner token and unexpired lease, then verify `active-release` is still `A`; if either check fails, release only a lock still owned by this publisher and abort because another publisher has won. A future compare-and-set implementation is acceptable only if it atomically claims the transition from `A` before either pointer write.
8. Revalidate the owner token and lease immediately before writing `previous-release = A` and again before its read-back verification. If the write or verification is ambiguous, revalidate ownership, confirm `active-release` remains `A`, and restore/reconcile `previous-release = P` before releasing the lock. Revalidate ownership again immediately before writing `active-release = B` and before its read-back verification. If the final write is ambiguous, revalidate ownership and reconcile the pointers to either the complete promoted state (`previous-release = A`, `active-release = B`) or the complete pre-promotion state (`previous-release = P`, `active-release = A`). If ownership or lease validation fails at any point, stop all pointer writes, surface the observed state, re-read both pointers, and begin a new promotion attempt; a stale writer must never repair or overwrite state after losing its claim. The two pointers must not both be advanced to `B`, or rollback would be ineffective.
