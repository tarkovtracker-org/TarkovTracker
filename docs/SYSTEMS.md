...
sub-quests, so it groups routes and gates chapter-level bulk completion but never disables an
individual objective toggle. Declared `endings` replace text-derived ending labels when present, an
ending with no attributed objectives reports pending evidence rather than being hidden, and
exclusivity between endings is not inferred. `referenceCoverage.partial` surfaces as a chapter badge
because a missing objective is not evidence that none exists. Objective IDs are upstream-owned:
persisted marks whose IDs the current chapter no longer defines (overlay v1.93 re-keyed all of The
Ticket's) are reported for re-checking, never silently remapped or deleted.
